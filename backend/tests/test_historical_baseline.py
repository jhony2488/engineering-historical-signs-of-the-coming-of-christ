...
def test_baseline_seed_loads_statistics():
    ...
    assert stats["total_profecias_biblicas"] == 1817
    assert stats["eventos_principais_mapeados"] == 600
    assert stats["profecias_pendentes"] == 20
...
def test_baseline_initialize_idempotent():
    ...
    assert first["estatisticas"]["total_profecias_biblicas"] == 1817
...
def test_baseline_payload_structure():
    ...
    assert len(payload["profecias_pendentes"]) == 20
