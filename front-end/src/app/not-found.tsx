...
export default function NotFound() {
  return (
    <>
      <Header />
      <main className="mx-auto flex min-h-[calc(100vh-5rem)] max-w-7xl flex-col items-center justify-center px-4 py-16 sm:px-6">
        ...
        >
          {ROTAS.map((rota) => (
            <Link
              key={rota.href}
              href={rota.href}
              className="rounded-lg border border-ink-700/80 bg-ink-900/50 px-4 py-3 text-center hover:border-ink-600 hover:bg-ink-900 transition"
            >
